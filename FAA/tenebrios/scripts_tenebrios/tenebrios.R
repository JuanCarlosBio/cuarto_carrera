...
result_o4 <- read_excel("practica 1 aiai/tenebrios.xlsx", # Puedes importar fácilmente la tabla de excel en el "Import Dataset" de la derecha, si te has descargado el csv de mi cuenta :)
                        sheet = "resultados_nuestros") %>% 
  mutate(dia=seq(0,77,7))

url1 <- "https://raw.githubusercontent.com/Juankkar/tenebrios_faa/main/bases_datos/tenebrios.csv"
result_o4 <- read.csv(url1, sep = ";"); colnames(result_o4) <- c("semana",        # Semana de estudio
...
result_o4 <- result_o4 %>% mutate(masa.corporal = str_replace(masa.corporal, pattern = ",", replacement = "."),
                                  tasa.crec.s = str_replace(tasa.crec.s, pattern = ",", replacement = "."),
                                  tasa.crec.d = str_replace(tasa.crec.d, pattern = ",", replacement = "."),
                                  dia = seq(0,77,7))

result_o4$masa.corporal <- as.numeric(result_o4$masa.corporal)
result_o4$tasa.crec.d <- as.numeric(result_o4$tasa.crec.d)
result_o4$tasa.crec.s <- as.numeric(result_o4$tasa.crec.s)
...
p1 <- masa_tasa_o4 %>% 
  mutate(masa_tasa = factor(masa_tasa,
                            ...
                            labels = c("Masa Corporal", "Tasa Crecimiento"))) %>%    # que los otros no eran estéticos
...
url2 <- "https://raw.githubusercontent.com/Juankkar/tenebrios_faa/main/bases_datos/df_o4.csv"
